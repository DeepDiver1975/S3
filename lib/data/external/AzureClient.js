...
const { logHelper, translateMetaHeaders } = require('./utils');
...
class AzureClient {
    ...
    _getMetaHeaders(objectMD) {
        ...
        return translateMetaHeaders(metaHeaders);
    }
    ...
    put(stream, size, keyContext, reqUids, callback) {
        ...
        keyContext.objectKey, this._dataStoreName, log, err => {
            ...
            const options = { metadata:
                translateMetaHeaders(keyContext.metaHeaders,
                keyContext.tagging) };
            if (size === 0) {
    ...
    sourceLocationConstraintName, storeMetadataParams, log, callback) {
        ...
        if (storeMetadataParams.metaHeaders) {
            options = { metadata:
            translateMetaHeaders(storeMetadataParams.metaHeaders) };
        }
