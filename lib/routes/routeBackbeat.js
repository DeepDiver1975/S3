...
function handleTaggingOperation(request, response, method, dataStoreVersionId,
    log, callback) {
    ...
    return multipleBackendGateway.objectTagging(method, request.objectKey,
    request.bucketName, objectMD, log, err => {
        if (err) {
            log.error(`error during object tagging: ${method}`, {
                error: err,
